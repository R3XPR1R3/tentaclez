...
> **Статус (v0.2)**: текущий код требует SnapTrade-ключей в `.env`. Следующая итерация — настройка из Telegram + один скрипт-установщик для Raspberry Pi. Полное ТЗ для следующего этапа: [`TZ.md`](TZ.md).
